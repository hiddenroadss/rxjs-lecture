...
Спостережувані об'єкти(далі - Спостережувані) — це ліниві Push-колекції кількох значень. Вони заповнюють пропущене місце в наступній таблиці:
| | Single | Multiple |
| --- | --- | --- |
| Pull | Функція | Ітератор |
| Push | Обіцянка | **Спостережуваний** |

Приклад. Нижче наведено Спостережуваний, який надсилає значення 1, 2, 3 негайно (синхронно) під час підписки, а значення 4 після того, як мине одна секунда після виклику підписки, а потім завершується:
```javascript
...
```
Саме по собі, створення Спостережуваного нічого не робить, для того, щоб почати отримувати дані, нам потрібно на нього підписатися:
```javascript
import { Observable } from 'rxjs';
...
RxJS представляє Спостережувані(Observables), нову систему Push для JavaScript. Спостережуваний є виробником колекції значень, «виштовхуючи» їх до спостерігачів (споживачів).
...
- Спостережуваний — це обчислення з лінивим виконанням, яке може синхронно або асинхронно повертати від нуля до (потенційно) нескінченної кількості значень з моменту його виклику.
