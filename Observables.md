...
ES2015 представив функції генератора та ітератори (function*), інший тип Pull-системи. Код, який викликає iterator.next(), є споживачем, який «витягує» кілька значень з ітератора (виробника).


Що таке Push? У системах Push виробник визначає, коли надсилати дані споживачеві. Споживач не знає, коли він отримає ці дані.
Promises є найпоширенішим типом Push-системи в JavaScript сьогодні. Promise (виробник) надає дозволене значення зареєстрованим зворотним викликам (споживачам), але, на відміну від функцій, саме проміс відповідає за те, щоб точно визначити, коли це значення «виштовхується» до зворотних викликів.
RxJS представляє Observables, нову систему Push для JavaScript. Спостережуваний є виробником кількох значень, «виштовхуючи» їх до спостерігачів (споживачів).
Функція — це обчислення з лінивим обчисленням, яке синхронно повертає одне значення під час виклику. 
Генератор — це обчислення з лінивою оцінкою, яке синхронно повертає від нуля до (потенційно) нескінченних значень на ітерації.
Обіцянка — це обчислення, яке може (або не може) зрештою повернути одне значення. 
Observable — це обчислення з лінивою оцінкою, яке може синхронно або асинхронно повертати від нуля до (потенційно) нескінченних значень з моменту його виклику.
...
Anatomy of the observable
Observables створюються за допомогою new Observable або оператора створення, на них підписується спостерігач, виконуються для доставки сповіщень про наступне/помилку/завершення до спостерігача, і їхнє виконання може бути припинено. Усі ці чотири аспекти закодовані в екземплярі Observable, але деякі з цих аспектів пов’язані з іншими типами, наприклад Observer і Subscription.
Основні спостережувані проблеми:
Створення спостережуваних 
Підписка на Observables 
Виконання Observable 
Утилізація спостережуваних
