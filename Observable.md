...
2. вони виштовхують дані (самі контролюють коли споживач їх отримає)
3. вони ліниві

Для того, щоб краще зрозуміти перший пункт, ми можемо розглянути щось нам вже знайоме - масиви, які також є колекціями даних.

Єдина велика різниця, це те,
що масиви працють виключно _синхронно_, а Об'єкти Спостереження як _синхронно_, так і _асинхронно_(з'являється поняття часу).
...
Важливим моментом є те, що всі ці методи використовують основи функціонального програмування:
...
В даному випадку в нас є масив чисел, який передається в `map()`, всередині цього методу ми застосовуємо функцію `x => x + 1` до кожного елементу масиву.
...
В даному випадку ми розбили ланцюг на 2 частини. Спочатку ми додали до кожного елементу масиву одиницю. А потім новий масив `incrementedArr` відфільтрували 2 рази і отримали окремо масив з парними і непарними числами.
...
P.S.: Варто зазначити, що в версіях RxJS 6+ наші оператори не прикріплені до класу Observable і ми використовуємо їх не як `Observable.prototype.map`, а `Observable.prototype.pipe(map())`. Це було зроблено для покращення швидкості бібліотеки і можливості відкидати код, який ми не використовуємо.
...
Наприклад, коли ми робимо HTTP запит з `fetch` (який використовує Обіцянки всередині), то пишемо логіку, яку потрібно виконати після завершення запиту в методі `then`.

```js
fetch('someurl').then((data) => doSomething(data));
```

Ми не контролюємо, коли саме `fetch` завершиться і віддасть нам дані, ми просто описуємо, що треба зробити, коли це трапиться. Ми просто пасивні споживачі в даному випадку.
...
Саме по собі, створення Об'єкту Спостереження майже нічого не робить. Ми просто реєструємо функцію `subscribe` всередині стореного Об'єту Спостереження:
...
// Поки що абсолютно нічого не відбулося
```

Для того, щоб почати отримувати дані, нам потрібно на нього підписатися:
...
### Чи є виконання Об'єкту Спостереження асинхронним?
...
Усі ці чотири аспекти закодовані в екземплярі Об'єкта Спостереження, але деякі з цих аспектів пов’язані з іншими типами, наприклад Спостерігач і Підписка.W
...
Це суттєво відрізняється від WEB API обробників подій, таких як `addEventListener` / `removeEventListener`. З `observable.subscribe` даний Спостерігач не зареєстрований як слухач в Об'єкта Спостереження. Об'єкт Спостереження навіть не підтримує список приєднаних Спостерігачів.??Дописати як це влаштовано в обробниках подій
...
Так як кожне виконання є ексклюзивним лише для одного Спостерігача, щойно Спостерігач закінчить отримання значень, він повинен мати спосіб зупинити виконання, щоб уникнути марної витрати обчислювальної потужності або ресурсів пам’яті.
...
Кожен Об'єкт Спостереження повинен визначати, як розпоряджатися ресурсами цього виконання, коли ми створюємо Об'єкт Спостереження за допомогою `new Observable(function subscribe() {...})`. Ви можете зробити це, повернувши спеціальну функцію скасування підписки з функції, яку ми передаємо в конструктор `Observable`.
